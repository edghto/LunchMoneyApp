...
namespace LunchMoneyApp
{
    ...
    public class HttpPOSTWorker
    {
        ...
        static public Boolean invoke(String url, String postData, ProcessHttpPostResponse delagate)
        {
            HttpPOSTWorker httpPostWorker = new HttpPOSTWorker();
            try
            {
                httpPostWorker.connect(url,
                    Encoding.UTF8.GetBytes(postData), new ProcessHttpPostResponse(delagate));
            }
            catch
            {
                return false;
